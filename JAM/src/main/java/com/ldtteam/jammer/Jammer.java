...
import com.ldtteam.jam.jamspec.IJammer;
...
import com.ldtteam.jam.jamspec.configuration.MappingRuntimeConfiguration;
import com.ldtteam.jam.jamspec.configuration.OutputConfiguration;
import com.ldtteam.jam.jamspec.mapping.MappingResult;
import com.ldtteam.jammer.loader.ASMDataLoader;
import com.ldtteam.jammer.loader.LoadedASMData;
...
public class Jammer implements IJammer
{
    ...
    public void run(final Configuration configuration)
    {
        ...
        Map<ClassNode, LinkedList<HistoricalClassMapping>> transitiveClassMappings = buildTransitiveClassMappings(mappedClasses, transitionMappings.values());
        ...
        final LoadedASMData targetASMData = dataByInputName.get(Objects.requireNonNull(configuration.inputs().peekLast()).name());
    ...
    Map<ClassNode, LinkedList<HistoricalClassMapping>> buildTransitiveClassMappings(final BiMap<ClassNode, ClassNode> currentClassMappings, final Collection<JarMappingResult> mappings)
    {
        ...
        final Map<ClassNode, LinkedList<HistoricalClassMapping>> transitiveClassMappings = Maps.newHashMap();
    ...
    private BiMap<MethodNode, MethodNode> mapMethodsTransitively(final Set<MethodNode> unmappedMethods, final Map<MethodNode, ClassNode> methodOwners, final Map<ClassNode, LinkedList<HistoricalClassMapping>> history, MappingRuntimeConfiguration runtimeConfiguration)
    {
        ...
        unmappedMethodsByOwner.keySet().forEach(
          nextGenClass -> {
              final LinkedList<HistoricalClassMapping> workingHistory = getAdditionalHistoryOfClass(history, nextGenClass);
    ...
    private BiMap<FieldNode, FieldNode> mapFieldsTransitively(final Set<FieldNode> unmappedFields, final Map<FieldNode, ClassNode> fieldOwners, final Map<ClassNode, LinkedList<HistoricalClassMapping>> history, MappingRuntimeConfiguration runtimeConfiguration)
    {
    ...
    private LinkedList<HistoricalClassMapping> getAdditionalHistoryOfClass(final Map<ClassNode, LinkedList<HistoricalClassMapping>> history, final ClassNode classNode) {
        final LinkedList<HistoricalClassMapping> historyForNextGenClass = history.getOrDefault(classNode, new LinkedList<>());
