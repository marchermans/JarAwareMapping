...
public class JammerRuntime
{
    public static final Logger LOGGER = LoggerFactory.getLogger(JammerRuntime.class);

    private final IRemapperProducer officialToObfuscatedRemapperProducer;
    ...
    public void run(String[] args)
    {
        ...
        for (int i = 0; i < existingNames.size(); i++)
        {
            ...
            final Optional<IRemapper> remapped = Optional.of(officialToObfuscatedRemapperProducer.from(existingMappings.get(i).toPath()));
    ...
    public interface IRemapperProducer {
        ...
        IRemapper from(final Path path);
    }

    @FunctionalInterface
    public interface IExistingIdentitySupplierProducer {
        IExistingIdentitySupplier get();
    }
}
